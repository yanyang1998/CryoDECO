...
from Data import mask as masking
        ...
        if apply_tilting_scheme:
            ...
            if n_in_planes > 1:  # first step
                ...
                for chunk_idx in range(n_in_planes * n_tilts):
                    y = model.eval_on_slice(x[:, :, chunk_idx],hypervolume, route_labels=route_labels[:, :, chunk_idx],z=z)
                    y = y.float()  # [b, nq, npts]
                    if z is None:
            ...
            # z: [b, z_dim], [b * nq, z_dim]
            y_hat = model.eval_on_slice(x,hypervolume,route_labels=route_labels, z=z)
...
def opt_theta_trans(
    ...
    best_loss = loss[keep_bn, keep_t, keep_q]
